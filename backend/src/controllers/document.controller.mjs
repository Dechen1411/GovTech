...
export const submit = async ({ db, req, body, res }) => {
  ...
  if (!user.walletAddress || !isWallet(user.walletAddress)) {
    throw httpError(400, "Complete NDI login so the platform can issue a wallet before submitting property documents");
  }
  if (!isVerifiedWallet(db, user.walletAddress)) {
    throw httpError(400, "Your platform wallet must be NDI-verified before submitting property documents");
  }
